...
//        gradient.endPoint = CGPoint(x: 1.0, y: 1.0)
    ...
    public func setGradientBlackImage() {
        ...
        topGradient.locations = [0.0, 0.1]
        topGradient.frame = self.bounds
        self.layer.insertSublayer(topGradient, at: 0)
        
        let gradient = CAGradientLayer()
        gradient.colors = [UIColor.systemBackground.withAlphaComponent(0.0).cgColor, UIColor.secondarySystemBackground.withAlphaComponent(0.5).cgColor]
        gradient.locations = [0.6, 1.0]
        gradient.frame = self.bounds
        self.layer.insertSublayer(gradient, at: .min)
    }
